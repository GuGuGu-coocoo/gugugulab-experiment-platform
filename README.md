...
2026-09-23 本轮修复（账号永久删除、邀请身份绑定、稳定审计主体、研究者密码夹具与浏览器连接竞态）在 `GEP_T17_BROWSER=1` 下完成全量回归：**611 项通过、0 失败**（含真实 Chrome 用例）。此前批次的 44 项定向服务端测试与 1 项真实 Chrome 浏览器测试、原版本服务端 **56 项**、发行准备 **2 项**及贯通 **31 项**属于不同范围，不能相加当作一次完整回归。环境、复现命令及限制见[验收记录](docs/verification.md)。
