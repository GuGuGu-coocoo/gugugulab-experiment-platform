...
更新于 **2026-09-24**。Phase 01、02 的有限合成工程验收完成；Phase 03 预演暴露差距的工程修复、受影响集成回归与 Windows x64 实机工程运行已完成。**Phase 03 尚未完成：2026-09-21 已进行非开发者人工测试并形成修复反馈；修复与完整工程回归仍在进行，尚未进行新一轮人工测试。**
...
| Phase 03F | 集成回归、Windows 原生实机验收与人工测试 | 工程编排、隔离回归与旧版构建的 **Windows x64 实机 WN01–WN06 工程门槛**已完成（严格门槛 `RUNTIME_PASS`）；R11 新版构建改为完整证据门槛（汇总 PASS 不接受），本机复验通过，真实 Windows 实机运行待 R11W；2026-09-21 人工测试已发生并形成修复反馈，修复后的完整回归与新人工测试尚未进行 |
...
- GEC 本地结果导出与保存反馈（已通过本轮工程验证）：本地测试结果 JSONL 只在 SDK/后端内导出本机当前未锁定的本地会话，缺失/已清理/锁定/远端一律拒绝且不产生下载；原生保存先在所选目录写同目录临时文件、核对完整内容后原子替换旧目标，打开后写入/flush 失败报 `results_export_unavailable` 且旧文件与本机记录保留；保存提示与完成状态独立（未完成时不宣称“本地测试完成”），目录反馈和“打开结果目录”指向实际最终文件父目录；仅数据恢复也会永久退役前置表单，已退役入口在保存失败后仍不可重新准入。
- 2026-09-24 R11AR 证据链修正（工程侧进行中，待新一轮人工测试）：Windows 运行只接受完整证据——报告格式、当前程序源码摘要与输入集合、冻结程序/kit/运行标识、原始 harness 运行文档、设备 OS/架构/版本与交互会话、逐事件对账导出，全部按文件重新核对；仅含六个 PASS 的汇总 JSON 被拒绝，旧 kit 被源码绑定拒绝，不能继承旧结果。程序源码绑定现覆盖四个 `packages/gec_web/*.js` 与实际打包输入，构建前后不一致拒绝；解包 macOS 运行资源（可执行文件、`.pck`、动态库）与绑定归档逐成员比对；三个绑定入口与 Windows kit 清单拒绝祖先链接，负向测试证明外部 canary 与旧产物字节不变、不启动任何程序。本机 `--verify-local` 复验通过（`tests/remediation` **262**、受影响旧套件 **438**、shell **133**、package **112**、browser **20** 用例，零失败零跳过），仅真实 Windows WN01–WN06 与人工测试待 R11W；Phase 03 未完成。复现命令与边界见[验收记录](docs/verification.md)。
- GEC 补传轮次与失败收尾、新版三平台整体回归及人工测试仍待完成；当前不宣称 Phase03 全部交付。
...
2026-09-24 实验数据接口新增默认事件类型、schema 与显式字段／快照回调声明，保留原有四参数记录方式。记录前拒绝循环和超深数据，保留原值与调用时快照，不接管科学任务。该批次真实 Godot、SQLite、Chrome/IndexedDB 与隔离 GEP 验证 **11 项通过**，关联回归 **47 项通过**，另有循环输入独立探针通过（均无失败、无跳过）；新版完整包集成、Windows 实机回归和人工复测仍待后续。用法与各层保存边界见[实验开发者说明](docs/experiment_developer.md)。
