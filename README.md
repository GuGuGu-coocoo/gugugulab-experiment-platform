...
更新于 **2026-09-21**。Phase 01、02 的有限合成工程验收完成；Phase 03 预演暴露差距的工程修复、受影响集成回归与 Windows x64 实机工程运行已完成。**Phase 03 尚未完成：原设计者自主体验与独立 T17 仍未运行。**
...
| Phase 03B | Owner/Admin/普通用户、账号生命周期、实例权限矩阵、Excel 预览导入 | 已实现并通过定向工程验证（T26 工程证据；人工验收未运行） |
| Phase 03C | 研究公开设置、唯一当前招募发行、旧会话兼容 | 已实现并通过定向工程验证（含只读迁移演练） |
| Phase 03D | GEC 统一参与壳、短恢复码、macOS arm64／Windows x64 完整冻结原生包 | macOS 完整包与 Web 壳已实现并有合成工程闭环；Windows x64 平台契约、交叉构建与**真实实机执行**均有工程证据（WN01–WN06 一次完整运行 236 项 0 失败）；设计者体验未运行 |
| Phase 03E | 模块化后台、主题与中英文、被试研究门户 | 已实现并通过定向工程验证 |
| Phase 03F | 集成回归、Windows 原生实机验收、原设计者自主体验、非开发者独立 T17 | 工程编排、隔离回归与 **Windows x64 实机 WN01–WN06 工程门槛**已完成（严格门槛 `RUNTIME_PASS`）；原设计者自主体验与独立 T17 仍为 NOT_RUN |
...
已实现独立账号治理、权限矩阵、预览确认及有界 Excel 导入；研究支持显式公开政策、当前招募发行和稳定入口 `/join/<study_id>`。切换发行不改写已有会话与待上传数据；过期入口要求刷新。旧研究迁移默认私有且不自动选择发行。统一 GEC 参与壳与完整冻结原生包已交付工程版；原设计者自主体验及独立 T17 尚未通过，当前仅用于合成验收。
