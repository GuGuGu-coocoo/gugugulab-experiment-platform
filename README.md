...
已修复 Web 提示文字从两行变为一行时输入框遮挡 Start 的问题：固定提示区最小高度，初始、试次、错误和完成状态保持布局一致。Godot 布局回归及 17 项发行/端到端测试通过，Web/macOS 已重新构建；已有批准发行不被原地替换。

原生窗口恢复框的自动输入仍未可靠通过，保留为未完成项；现有命令行恢复及数据一致性测试不能替代该界面验收。此次未放宽凭据校验，也未宣称 Phase 01–03 全部完成。
