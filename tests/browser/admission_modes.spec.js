...
test('GUI roster policies and real API enforce all three admission modes',async({page,request})=>{
 ...
 for(const mode of ['anonymous','id','password']){
  await page.goto('http://admin.localhost:8000/');await page.locator('[name=title]').fill(mode+' synthetic '+Date.now());await page.getByRole('button',{name:'创建',exact:true}).click();
  ...
  await page.goto(studyUrl+'/builds');await page.locator('[name=descriptor]').fill(fs.readFileSync('build/native/descriptor.json','utf8'));await page.getByRole('button',{name:'登记不可变构建'}).click();await page.getByRole('button',{name:'批准合成发行'}).click();
  const configURL=await page.getByRole('link',{name:'导出连接配置'}).getAttribute('href');const config=await (await page.request.get('http://admin.localhost:8000'+configURL)).json();
