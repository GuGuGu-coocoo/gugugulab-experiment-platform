...
test('native authorized data-only recovery unpauses uploads without replaying trials',async({page})=>{
 ...
 execFileSync('.venv/bin/python',['-c','import sqlite3,json,sys;c=sqlite3.connect(sys.argv[1]);s=json.loads(c.execute("select value from sessions").fetchone()[0]);s["checkpoint"].pop("strategy");s.update(paused=True,attempts=32,retry_at=9999999999,pending=[e["event_id"] for e in s["records"]]);c.execute("update sessions set value=?",[json.dumps(s)]);c.commit()',path.join(dir,'queue.sqlite')]);
 ...
 const after=snapshot();expect(after.id).toBe(saved.id);expect(after.records).toEqual(saved.records);expect(after.segments).toEqual(saved.segments);expect(after.pending).toEqual([]);
});
