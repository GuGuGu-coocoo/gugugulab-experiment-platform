...
// Isolated instance only: the connection comes from the orchestrator (GEP_ISO_*),
// never from the dev instance or the protected acceptance database.
...
test('native new participation protects old data and bounded buffer rejects overflow',async()=>{
 ...
 const proc=spawn('godot',['--headless','--path','examples/synthetic_experiment','--script',path.resolve('tests/native/shared_device_harness.gd')],{env:{...process.env,GEP_SYNTHETIC_STORAGE:storage,GEP_TEST_CONFIG:path.resolve(target.connection)},stdio:['ignore','pipe','pipe']});
