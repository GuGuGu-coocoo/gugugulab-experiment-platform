...
test('register native build and export public config through GUI',async({page})=>{
 ...
 await page.goto(studyUrl+'/builds');await page.locator('[name=descriptor]').fill(fs.readFileSync('build/native/descriptor.json','utf8'));await page.getByRole('button',{name:'登记不可变构建'}).click();
 ...
 fs.writeFileSync('build/native/study_url.txt',studyUrl);
});
