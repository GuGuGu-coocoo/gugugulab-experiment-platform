...
def _fake_world(tmp_path, monkeypatch):
    ...
    for index, mode in enumerate(native.MODES):
        ...
        _store_copy(store_dir / "queue.sqlite", [_session_document(session_id, events)])
        _db_session(connection, session_id, modes[mode]["release_id"], events)
        export = evidence / label / "export.jsonl"
        export.write_text("\n".join(json.dumps({"study_id": modes[mode]["study_id"],
                                                "release_id": modes[mode]["release_id"],
                                                "build_id": modes[mode]["build_id"], "record": event})
                                    for event in events))
        cases[1]["evidence"].setdefault("modes", {})[mode] = {
            ...
            "export_values": [217.25, 321.5], "export_path": f"evidence/{label}/export.jsonl",
            "export_sha256": hashlib.sha256(export.read_bytes()).hexdigest(),
            "store_copy": {"queue.sqlite": f"evidence/{label}/queue.sqlite"}}
        cases[4]["evidence"].setdefault("modes", {})[mode] = {
            ...
            "export_path": f"evidence/{label}/export.jsonl",
            "export_sha256": hashlib.sha256(export.read_bytes()).hexdigest()}
    # WN03
    offline_dir = evidence / "WN03-offline"
    offline_dir.mkdir()
    ...
    _store_copy(offline_dir / "queue.sqlite", [_session_document(offline_session, offline_events, pending=["a"])])
    reconnect_dir = evidence / "WN03-reconnect"
    reconnect_dir.mkdir()
    _store_copy(reconnect_dir / "queue.sqlite", [_session_document(offline_session, offline_events)])
    _db_session(connection, offline_session, modes["password"]["release_id"], offline_events)
    wn03_export = evidence / "WN03" / "export.jsonl"
    wn03_export.parent.mkdir(parents=True)
    wn03_export.write_text("\n".join(json.dumps({"study_id": modes["password"]["study_id"],
                                                 "release_id": modes["password"]["release_id"],
                                                 "build_id": modes["password"]["build_id"], "record": event})
    ...
    short_events = [_event(f"40000000-0000-0000-0000-00000000000{position}", 321.5, session_id=short_session)
                    for position in range(4)]
    _store_copy(short_dir / "queue.sqlite", [_session_document(short_session, short_events)])
    ...
    cases[3]["evidence"].update({
        "short_code": {"session_id": short_session,
                       ...
                       "replay_exit": 2, "store_copy": {"queue.sqlite": "evidence/WN04-short-code/queue.sqlite"}},
    ...
    old_dir = evidence / "WN06-old-release"
    old_dir.mkdir()
    ...
    old_events = [_event(f"60000000-0000-0000-0000-00000000000{position}", 321.5, session_id=old_session)
                  for position in range(4)]
    _store_copy(old_dir / "queue.sqlite", [_session_document(old_session, old_events)])
    _db_session(connection, old_session, modes["password"]["release_id"], old_events)
    old_export = evidence / "WN06" / "export.jsonl"
    ...
    cases[5]["evidence"]["old_release"] = {
        ...
        "export_path": "evidence/WN06/export.jsonl",
        "export_sha256": hashlib.sha256(old_export.read_bytes()).hexdigest(),
        "export_event_ids": [event["event_id"] for event in old_events],
        "store_copy": {"queue.sqlite": "evidence/WN06-old-release/queue.sqlite"}}
...
def test_gate_reconciles_the_native_store_copy_against_the_bound_instance(tmp_path, monkeypatch):
    ...
    prep, run_dir, _modes, _descriptor = _fake_world(tmp_path, monkeypatch)
    report = json.loads((run_dir / "run.json").read_text())
    store = run_dir / "evidence" / "WN02-id" / "queue.sqlite"
    ...
    result = native.validate_run(run_dir, prep)
    assert result["verdict"] == native.GATE_VERDICT_INVALID
    assert any("native store copy" in problem and "differs" in problem for problem in result["problems"])
...
def test_gate_reconciles_the_old_release_store_copy(tmp_path, monkeypatch):
    prep, run_dir, _modes, _descriptor = _fake_world(tmp_path, monkeypatch)
    store = run_dir / "evidence" / "WN06-old-release" / "queue.sqlite"
    connection = sqlite3.connect(store)
    row_id, raw = connection.execute("SELECT id,value FROM sessions").fetchone()
    document = json.loads(raw)
    ...
    result = native.validate_run(run_dir, prep)
    assert result["verdict"] == native.GATE_VERDICT_INVALID
    assert any("WN06 old release" in problem and "differs" in problem for problem in result["problems"])
...
def test_interactive_launch_waits_only_for_readiness_never_the_program(tmp_path, monkeypatch):
    ...
    spawned = []
    monkeypatch.setattr(harness, "spawn_launcher", lambda script: spawned.append(Path(script)) or launcher)
...
def test_interactive_failure_releases_only_the_owned_launcher(tmp_path, monkeypatch):
    ...
    harness = native_harness.Harness(run_root=tmp_path / "run")
    launcher = FakeLauncher()
    monkeypatch.setattr(harness, "spawn_launcher", lambda script: launcher)
...
def test_scheduled_task_and_script_survive_until_the_launch_is_closed(tmp_path, monkeypatch):
    ...
    assert launch.task_name is None and not launch.script_path.exists() and launch.closed
...
def test_launch_script_quotes_paths_with_spaces_and_apostrophes(tmp_path, monkeypatch):
    ...
    launcher = FakeLauncher()
    spawned = []
    monkeypatch.setattr(harness, "spawn_launcher", lambda path: spawned.append(Path(path)) or launcher)
    monkeypatch.setattr(harness, "wait_for", lambda path, timeout: dict(LAUNCH_RECORD))
