...
状态 / Status：**2026-09-21 已进行非开发者人工测试并形成修复反馈；本文清单的人工体验 = NOT_RUN；T17 编号保留，帮助/失败/未运行分别如实记录**。本文是准备说明与记录模板，不是通过报告，
也不代替人工测试（见 [researcher_acceptance.md](researcher_acceptance.md)）。
...
**仍未运行**：设计者在 Windows 上双击后的实际操作、界面体验与本人填写结果（人工测试尚未进行，T17 编号保留、结果如实记录）。
...
- 使用的包与摘要前缀：未运行
- 需要维护者解释的步骤：未运行
- 结论：**未运行**（人工测试由维护者按修复与工程回归就绪情况安排；T17 编号保留，帮助/失败/未运行分别如实记录）
