...
Shared-device/buffer coverage: both backends reject the 65th uncommitted event without changing the first 64 or claiming local persistence. Native real SQLite/API tests preserve the previous active checkpoint, deny its front recovery export after new participation and complete a separate 64-record queue in batches. A discovered extra front-lock field on already cleaned tombstones was removed from both new-admission paths; ten relevant browser/native tests passed.
