...
Docker、独立操作人及剩余故障矩阵仍待验。第二实体 Windows 的基础 LAN 场景已验证，不等于完整故障矩阵通过。Compose 初始化／运行验收、完整限额故障矩阵、原生恢复数据导出实测、原生清理中断全矩阵仍未交付完整验收。真实研究、生产部署、备份责任、计时、隐私／撤回／保留政策、MFA 与发行签名另行批准。当前版本不可用于真实收数。
